...
def to_domain_suffix_list(domains: Iterable[str]) -> str:
    """Clash/Surge-style `DOMAIN-SUFFIX,<domain>` list, one rule per line."""
    return "\n".join(f"DOMAIN-SUFFIX,{d}" for d in sorted(domains)) + "\n"
...
def main() -> int:
    ...
    out_list_path = out_dir / "ads.list"
    ...
    out_list_path.write_text(to_domain_suffix_list(seen), encoding="utf-8")
    print(f"ads: {len(seen)} unique domains -> {out_list_path}")
